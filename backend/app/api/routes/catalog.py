...
async def shared_image_content(product_id: UUID, market_id: UUID = Depends(get_current_market_id), session: AsyncSession = Depends(get_catalog_session)):
    ...
    image = next((item for item in (product.images if product else []) if item.is_primary and (item.quality_status or "good") in {"good", "excellent"} and item.storage_key), None) if product else None
    if image is None:
