...
def _render_supermarket_payload_html(payload: dict[str, Any], *, generated_at: datetime) -> str:
    ...
    cards = "".join(_supermarket_card(item, config, density) for item in items)
...
def _supermarket_card(item: dict[str, Any], config: dict[str, Any], density: dict[str, Any]) -> str:
    ...
    image = f'<div class="promo-card-image">{_image(item)}</div>' if config["show_product_image"] else ""
    ...
    emphasis = "featured" if item.get("emphasis") == "featured" else "normal"
    return f'<article class="product-card" data-emphasis="{emphasis}">{image}<div class="price-panel"><span class="price">{price}</span>{badge}{old}</div><span class="brand-label">{brand}</span>{name}{package}</article>'
