...
@pytest.mark.asyncio
async def test_market_scope_dependency_reads_x_market_id_header() -> None:
    market_id = uuid4()

    assert await get_current_market_id(market_id) == market_id
    assert await get_current_market_id(None) is None
...
async def test_global_brand_api_crud_runs_when_test_database_url_is_configured() -> None:
    ...
    try:
        ...
        async with AsyncClient(
            ...
            list_response = await async_client.get("/api/catalog/brands", params={"search": unique_name})
            assert list_response.status_code == 200
            assert list_response.json()["total"] >= 1

            delete_response = await async_client.delete(f"/api/catalog/brands/{brand['id']}")
