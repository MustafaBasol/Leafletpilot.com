...
def supermarket_fixture_payload(count: int, names: list[str]) -> dict:
    payload = fixture_payload(count, names)
    payload["template_slug"] = f"supermarket-promo-{count}"
    payload["template_name"] = "Supermarket Promo"
    payload["title"] = "Big weekly supermarket PROMO"
    payload["template_config"] = {"show_old_price": True, "show_payment_icons": True, "show_additional_logos": True}
    return payload

def write_branding_assets(root: Path) -> None:
    branding = root / "branding"; branding.mkdir(parents=True, exist_ok=True)
    assets = {"market-logo.svg": '<svg xmlns="http://www.w3.org/2000/svg" width="240" height="72"><path d="M18 52 38 12l20 40H18Z" fill="#ffd928"/><path d="m38 12 20 40h-8L38 28 26 52h-8Z" fill="#fff"/><text x="72" y="33" font-family="Arial" font-size="24" font-weight="900" fill="#fff">DEMO</text><text x="72" y="57" font-family="Arial" font-size="20" font-weight="900" fill="#ffd928">MART</text></svg>', "fresh.svg": '<svg xmlns="http://www.w3.org/2000/svg" width="108" height="48"><circle cx="24" cy="24" r="18" fill="#6ab04c"/><path d="M16 28c10-20 22-14 24-8-9 0-16 3-24 8Z" fill="#fff"/><text x="51" y="30" font-family="Arial" font-size="16" font-weight="900" fill="#fff">FRESH</text></svg>', "value.svg": '<svg xmlns="http://www.w3.org/2000/svg" width="108" height="48"><path d="M7 24 20 8l13 16-13 16L7 24Z" fill="#ffd166"/><text x="42" y="30" font-family="Arial" font-size="16" font-weight="900" fill="#fff">VALUE</text></svg>', "card.svg": '<svg xmlns="http://www.w3.org/2000/svg" width="80" height="44"><rect x="5" y="5" width="70" height="34" rx="8" fill="#264653"/><circle cx="28" cy="22" r="9" fill="#e9c46a"/><circle cx="51" cy="22" r="9" fill="#e76f51"/></svg>', "cash.svg": '<svg xmlns="http://www.w3.org/2000/svg" width="80" height="44"><path d="M8 8h64v28H8z" fill="#457b9d"/><circle cx="40" cy="22" r="10" fill="#a8dadc"/><path d="M40 13v18M31 22h18" stroke="#1d3557" stroke-width="3"/></svg>'}
...
def main() -> int:
    ...
    for count in (4, 9, 16):
        html = render_render_payload_html(supermarket_fixture_payload(count, names), generated_at=datetime.now(UTC))
        png = output / f"supermarket-promo-{count}.png"; pdf = output / f"supermarket-promo-{count}.pdf"
        render_html_to_png_sync(html, png); render_html_to_pdf_sync(html, pdf); checks = browser_checks(html, count)
        report = {"layout": f"supermarket-promo-{count}", "png": str(png), "pdf": pdf_metadata(pdf), "png_dimensions": png_dimensions(png), **checks, "price_panels": html.count('class="price-panel"'), "price_panel_opaque": "background:#ffd928" in html, "promo_header": "PROMO" in html, "quantity_fields": html.count('class="product-unit"')}
        required = [report["png_dimensions"] == [2480,3508], report["pdf"]["pages"] == 1, report["pdf"]["portrait_a4"], report["cards"] == count, report["price_panels"] == count, report["price_panel_opaque"], report["promo_header"], report["quantity_fields"] == count, report["no_overflow"]]
        if not all(required): raise SystemExit(json.dumps(report, indent=2))
        reports.append(report)
