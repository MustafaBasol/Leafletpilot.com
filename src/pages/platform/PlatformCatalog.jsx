...
const emptyProduct = { name: "", short_name: "", barcode: "", brand_id: "", category_id: "", package_size: "", package_type: "", aliases: [], is_active: true };
...
export function PlatformCatalog() {
  ...
  const openProduct = (product) => {
    ...
    setImages(product.images || []); setNotice(""); setError(""); setPreview(null); setFile(null);
  };
  ...
  const saveProduct = async (event) => {
    ...
    try {
      const body = { ...form, name: form.name.trim(), barcode: form.barcode.trim() || null, aliases: form.aliases.map((item) => ({ alias: item })) };
  ...
  return <section className="platform-page">
    ...
    {tab === "products" && <form className="form-grid" onSubmit={saveProduct}><h2>{selected ? "Edit global product" : "Create global product"}</h2>
      ...
      <label>Brand<select value={form.brand_id || ""} onChange={(event) => update("brand_id", event.target.value || null)}><option value="">None</option>{brands.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label><label>Category<select value={form.category_id || ""} onChange={(event) => update("category_id", event.target.value || null)}><option value="">None</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
