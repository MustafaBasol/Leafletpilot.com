export const platformLocales = ["tr", "en", "fr", "de"];

export const platformLabels = {
  tr: {
    platformAdmin: "Platform Admin",
    platformAdminDescription: "LeafletPilot kontrol paneli için güvenli yönetici girişi.",
    email: "E-posta",
    password: "Şifre",
    signIn: "Giriş Yap",
    signingIn: "Giriş yapılıyor...",
    showPassword: "Şifreyi göster",
    hidePassword: "Şifreyi gizle",
    invalidCredentials: "Geçersiz e-posta veya şifre",
    operationFailed: "İşlem tamamlanamadı.",
    overview: "Genel Bakış",
    logout: "Çıkış",
    pendingSignup: "Bekleyen Başvuru",
    awaitingOwner: "Owner Bekleniyor",
    onboarding: "Kurulumda",
    readyMarket: "Hazır Market",
    suspended: "Askıya Alındı",
    lifecycle: "Yaşam Döngüsü",
    readiness: "Hazırlık Durumu",
    ownerInvitation: "Owner Daveti",
    activeUser: "Aktif kullanıcı",
    requiredSetup: "Gerekli kurulum",
    lastActivity: "Son etkinlik",
    profile: "Profil",
    legalName: "Yasal ad",
    location: "Konum",
    languageCurrency: "Dil / Para birimi",
    timeZone: "Zaman dilimi",
    contact: "İletişim",
    productCampaign: "Ürün / Kampanya",
    product: "Ürün",
    campaign: "Kampanya",
    member: "Üye",
    members: "Üyeler",
    operation: "Operasyon",
    trialEnds: "Deneme bitişi",
    status: "Durum",
    allStatuses: "Tüm durumlar",
    market: "Market",
    markets: "Marketler",
    marketList: "Market Listesi",
    open: "Aç",
    signupRequests: "Başvurular",
    signupRequestList: "Başvuru Listesi",
    signupOverview: "Ücretsiz deneme başvurularını inceleyin ve uygun olanları markete dönüştürün.",
    marketOverview: "Yaşam döngüsü, deneme ve kurulum durumlarını izleyin.",
    search: "Ara",
    contactPerson: "Yetkili",
    date: "Tarih",
    phone: "Telefon",
    language: "Dil",
    monthlyCampaigns: "Aylık kampanya",
    notes: "Not",
    review: "İncelemeye Al",
    reject: "Reddet",
    rejectionReason: "Ret nedeni",
    activate: "Aktifleştir",
    suspend: "Askıya Al",
    archive: "Arşivle",
    loading: "Yükleniyor...",
    yes: "Var",
    no: "Yok",
    complete: "Tamam",
    missing: "Eksik",
    unknownStatus: "Bilinmeyen durum",
    readinessReady: "Market yayına hazır.",
    platformControl: "Platform Kontrol",
    platformControlOverview: "Pilot müşteri onboarding ve market hazırlığını izleyin.",
    marketOperationsOverview: "Yaşam döngüsü, hazırlık, owner daveti ve kurulum durumlarını izleyin.",
    signupOperationsOverview: "Başvuruları inceleyin, onaylayın ve pilot markete dönüştürün.",
    allLifecycle: "Tüm yaşam döngüleri",
    allReadiness: "Tüm hazırlık durumları",
    noMarketsForFilters: "Bu filtrelerde market yok.",
    noSignupRequestsForFilters: "Bu filtrelerde başvuru yok.",
    created: "Oluşturuldu",
    updated: "Güncellendi",
    lastUpdated: "Son güncelleme",
    lastReason: "Son neden",
    reasonPrompt: "Neden girin",
    confirmArchiveMarket: "Market arşivlensin mi?",
    confirmStatusChangeSuffix: "yapılsın mı?",
    confirmRevokeOwnerInvitation: "Kullanılmamış owner daveti iptal edilsin mi?",
    confirmRejectSignup: "Bu başvuru reddedilsin mi?",
    confirmProvisionSignup: "Bu başvuru için market ve owner daveti oluşturulsun mu?",
    expires: "Bitiş",
    accepted: "Kabul",
    ownerEmail: "Owner e-posta",
    createInvitation: "Davet Oluştur",
    rotateInvitation: "Davet Döndür",
    revokeInvitation: "Davet İptal",
    oneTimeInviteLink: "Tek seferlik davet bağlantısı",
    lastSent: "Son gönderim",
    sendCount: "Gönderim sayısı",
    sendError: "Gönderim hatası",
    invitationEmailNotice: "Davet bağlantısı e-posta ile gönderilir; ham token panelde gösterilmez.",
    copy: "Kopyala",
    copyInvitationLink: "Davet bağlantısını kopyala",
    manualLinkCopied: "Davet bağlantısı panoya kopyalandı.",
    createInvitationLink: "Davet bağlantısı oluştur",
    manualDeliveryNotice: "E-posta gönderimi kapalı. Güvenli davet bağlantısını oluşturup owner ile manuel olarak paylaşabilirsiniz.",
    manualLinkNotFound: "Kullanılabilir davet bulunamadı. Daveti yenileyip tekrar deneyin.",
    manualLinkConflict: "Manuel teslim artık gerekli değil. Davet durumunu yenileyin.",
    manualLinkFailed: "Güvenli davet bağlantısı oluşturulamadı. Lütfen tekrar deneyin.",
    none: "Yok",
    activateOrContinue: "Aktifleştir / Sürdür",
    step: "adım",
    platformActivity: "Platform Aktivitesi",
    recentPlatformActivity: "Son Platform Aktivitesi",
    action: "Aksiyon",
    target: "Hedef",
    noActivity: "Aktivite yok.",
    noPlatformActivity: "Henüz platform aktivitesi yok.",
    reviewNotes: "İnceleme notu",
    approve: "Onayla",
    marketProvision: "Market Provision",
    provisionInviteNotice: "Davet bağlantısı yalnızca başarılı provision yanıtında gösterilir. E-posta gönderimi manuel olarak işaretlenir.",
    createMarketAndInvitation: "Market Oluştur ve Davet Üret",
    plan: "Abonelik Planı",
    currentPlan: "Mevcut plan",
    changePlan: "Planı Değiştir",
    savePlan: "Planı Kaydet",
    planChanged: "Plan güncellendi.",
    monthlyCampaignQuota: "Aylık kampanya kotası",
    monthlyExportQuota: "Aylık dışa aktarım kotası",
    privateProductLimit: "Özel ürün limiti",
    privateTemplateLimit: "Özel şablon limiti",
    brandingAssetLimit: "Marka varlığı limiti",
    unlimited: "Sınırsız",
    provisionPlan: "Başlangıç planı",
  },
  en: {
    platformAdmin: "Platform Admin",
    platformAdminDescription: "Secure admin access for the LeafletPilot control panel.",
    email: "Email",
    password: "Password",
    signIn: "Sign In",
    signingIn: "Signing in...",
    showPassword: "Show password",
    hidePassword: "Hide password",
    invalidCredentials: "Invalid email or password",
    operationFailed: "The operation could not be completed.",
    overview: "Overview",
    logout: "Logout",
    pendingSignup: "Pending Signup",
    awaitingOwner: "Awaiting Owner",
    onboarding: "Onboarding",
    readyMarket: "Ready Market",
    suspended: "Suspended",
    lifecycle: "Lifecycle",
    readiness: "Readiness",
    ownerInvitation: "Owner Invitation",
    activeUser: "Active user",
    requiredSetup: "Required setup",
    lastActivity: "Last activity",
    profile: "Profile",
    legalName: "Legal name",
    location: "Location",
    languageCurrency: "Language / Currency",
    timeZone: "Time zone",
    contact: "Contact",
    productCampaign: "Product / Campaign",
    product: "Product",
    campaign: "Campaign",
    member: "Member",
    members: "Members",
    operation: "Operations",
    trialEnds: "Trial ends",
    status: "Status",
    allStatuses: "All statuses",
    market: "Market",
    markets: "Markets",
    marketList: "Market List",
    open: "Open",
    signupRequests: "Signup Requests",
    signupRequestList: "Signup Request List",
    signupOverview: "Review free trial requests and convert eligible requests into markets.",
    marketOverview: "Track lifecycle, trial, and onboarding status.",
    search: "Search",
    contactPerson: "Contact",
    date: "Date",
    phone: "Phone",
    language: "Language",
    monthlyCampaigns: "Monthly campaigns",
    notes: "Notes",
    review: "Mark Reviewing",
    reject: "Reject",
    rejectionReason: "Rejection reason",
    activate: "Activate",
    suspend: "Suspend",
    archive: "Archive",
    loading: "Loading...",
    yes: "Yes",
    no: "No",
    complete: "Complete",
    missing: "Missing",
    unknownStatus: "Unknown status",
    readinessReady: "Market is ready to go live.",
    platformControl: "Platform Control",
    platformControlOverview: "Track pilot customer onboarding and market readiness.",
    marketOperationsOverview: "Track lifecycle, readiness, owner invitation, and onboarding status.",
    signupOperationsOverview: "Review signup requests, approve them, and convert them into pilot markets.",
    allLifecycle: "All lifecycle states",
    allReadiness: "All readiness states",
    noMarketsForFilters: "No markets match these filters.",
    noSignupRequestsForFilters: "No signup requests match these filters.",
    created: "Created",
    updated: "Updated",
    lastUpdated: "Last updated",
    lastReason: "Last reason",
    reasonPrompt: "Enter a reason",
    confirmArchiveMarket: "Archive this market?",
    confirmStatusChangeSuffix: "should be applied?",
    confirmRevokeOwnerInvitation: "Revoke the unused owner invitation?",
    confirmRejectSignup: "Reject this signup request?",
    confirmProvisionSignup: "Create the market and owner invitation for this signup request?",
    expires: "Expires",
    accepted: "Accepted",
    ownerEmail: "Owner email",
    createInvitation: "Create Invitation",
    rotateInvitation: "Rotate Invitation",
    revokeInvitation: "Revoke Invitation",
    oneTimeInviteLink: "One-time invitation link",
    lastSent: "Last sent",
    sendCount: "Send count",
    sendError: "Send error",
    invitationEmailNotice: "The invitation link is sent by email; raw tokens are not shown in the panel.",
    copy: "Copy",
    copyInvitationLink: "Copy invitation link",
    manualLinkCopied: "Invitation link copied.",
    createInvitationLink: "Create invitation link",
    manualDeliveryNotice: "Email delivery is disabled. Create the secure invitation link and share it with the owner manually.",
    manualLinkNotFound: "No usable invitation was found. Refresh or rotate the invitation, then try again.",
    manualLinkConflict: "Manual delivery is no longer required. Refresh the invitation status.",
    manualLinkFailed: "The secure invitation link could not be created. Please try again.",
    none: "None",
    activateOrContinue: "Activate / Continue",
    step: "step",
    platformActivity: "Platform Activity",
    recentPlatformActivity: "Recent Platform Activity",
    action: "Action",
    target: "Target",
    noActivity: "No activity.",
    noPlatformActivity: "No platform activity yet.",
    reviewNotes: "Review notes",
    approve: "Approve",
    marketProvision: "Market Provision",
    provisionInviteNotice: "The invitation link is shown only after a successful provision response. Email delivery is marked manually.",
    createMarketAndInvitation: "Create Market and Invitation",
    plan: "Subscription Plan",
    currentPlan: "Current plan",
    changePlan: "Change Plan",
    savePlan: "Save Plan",
    planChanged: "Plan updated.",
    monthlyCampaignQuota: "Monthly campaign quota",
    monthlyExportQuota: "Monthly export quota",
    privateProductLimit: "Private product limit",
    privateTemplateLimit: "Private template limit",
    brandingAssetLimit: "Branding asset limit",
    unlimited: "Unlimited",
    provisionPlan: "Starting plan",
  },
  fr: {
    platformAdmin: "Admin Plateforme",
    platformAdminDescription: "Accès administrateur sécurisé au panneau de contrôle LeafletPilot.",
    email: "E-mail",
    password: "Mot de passe",
    signIn: "Se connecter",
    signingIn: "Connexion...",
    showPassword: "Afficher le mot de passe",
    hidePassword: "Masquer le mot de passe",
    invalidCredentials: "E-mail ou mot de passe invalide",
    operationFailed: "L'opération n'a pas pu être terminée.",
    overview: "Vue d'ensemble",
    logout: "Déconnexion",
    pendingSignup: "Demande en attente",
    awaitingOwner: "Owner attendu",
    onboarding: "Configuration",
    readyMarket: "Market prêt",
    suspended: "Suspendu",
    lifecycle: "Cycle de vie",
    readiness: "État de préparation",
    ownerInvitation: "Invitation owner",
    activeUser: "Utilisateur actif",
    requiredSetup: "Configuration requise",
    lastActivity: "Dernière activité",
    profile: "Profil",
    legalName: "Nom légal",
    location: "Localisation",
    languageCurrency: "Langue / Devise",
    timeZone: "Fuseau horaire",
    contact: "Contact",
    productCampaign: "Produit / Campagne",
    product: "Produit",
    campaign: "Campagne",
    member: "Membre",
    members: "Membres",
    operation: "Opérations",
    trialEnds: "Fin d'essai",
    status: "Statut",
    allStatuses: "Tous les statuts",
    market: "Market",
    markets: "Markets",
    marketList: "Liste des markets",
    open: "Ouvrir",
    signupRequests: "Demandes",
    signupRequestList: "Liste des demandes",
    signupOverview: "Examinez les demandes d'essai gratuit et convertissez les demandes éligibles en markets.",
    marketOverview: "Suivez le cycle de vie, l'essai et la configuration.",
    search: "Rechercher",
    contactPerson: "Contact",
    date: "Date",
    phone: "Téléphone",
    language: "Langue",
    monthlyCampaigns: "Campagnes mensuelles",
    notes: "Notes",
    review: "Mettre en revue",
    reject: "Rejeter",
    rejectionReason: "Motif du rejet",
    activate: "Activer",
    suspend: "Suspendre",
    archive: "Archiver",
    loading: "Chargement...",
    yes: "Oui",
    no: "Non",
    complete: "Terminé",
    missing: "Manquant",
    unknownStatus: "Statut inconnu",
    readinessReady: "Le market est prêt à être publié.",
    platformControl: "Contrôle plateforme",
    platformControlOverview: "Suivez l'onboarding client pilote et la préparation du market.",
    marketOperationsOverview: "Suivez le cycle de vie, la préparation, l'invitation owner et la configuration.",
    signupOperationsOverview: "Examinez les demandes, approuvez-les et convertissez-les en markets pilotes.",
    allLifecycle: "Tous les cycles de vie",
    allReadiness: "Tous les états de préparation",
    noMarketsForFilters: "Aucun market pour ces filtres.",
    noSignupRequestsForFilters: "Aucune demande pour ces filtres.",
    created: "Créé",
    updated: "Mis à jour",
    lastUpdated: "Dernière mise à jour",
    lastReason: "Dernier motif",
    reasonPrompt: "Saisissez un motif",
    confirmArchiveMarket: "Archiver ce market ?",
    confirmStatusChangeSuffix: "doit être appliqué ?",
    confirmRevokeOwnerInvitation: "Révoquer l'invitation owner inutilisée ?",
    confirmRejectSignup: "Rejeter cette demande ?",
    confirmProvisionSignup: "Créer le market et l'invitation owner pour cette demande ?",
    expires: "Expiration",
    accepted: "Accepté",
    ownerEmail: "E-mail owner",
    createInvitation: "Créer l'invitation",
    rotateInvitation: "Renouveler l'invitation",
    revokeInvitation: "Révoquer l'invitation",
    oneTimeInviteLink: "Lien d'invitation à usage unique",
    lastSent: "Dernier envoi",
    sendCount: "Nombre d'envois",
    sendError: "Erreur d'envoi",
    invitationEmailNotice: "Le lien d'invitation est envoyé par e-mail ; le token brut n'est pas affiché.",
    copy: "Copier",
    copyInvitationLink: "Copier le lien d’invitation",
    manualLinkCopied: "Lien d’invitation copié.",
    createInvitationLink: "Créer le lien d’invitation",
    manualDeliveryNotice: "L’envoi par e-mail est désactivé. Créez le lien d’invitation sécurisé et partagez-le manuellement avec l’owner.",
    manualLinkNotFound: "Aucune invitation utilisable n’a été trouvée. Actualisez ou renouvelez l’invitation, puis réessayez.",
    manualLinkConflict: "La livraison manuelle n’est plus requise. Actualisez l’état de l’invitation.",
    manualLinkFailed: "Le lien d’invitation sécurisé n’a pas pu être créé. Réessayez.",
    none: "Aucun",
    activateOrContinue: "Activer / Continuer",
    step: "étape",
    platformActivity: "Activité plateforme",
    recentPlatformActivity: "Activité plateforme récente",
    action: "Action",
    target: "Cible",
    noActivity: "Aucune activité.",
    noPlatformActivity: "Aucune activité plateforme pour le moment.",
    reviewNotes: "Notes de revue",
    approve: "Approuver",
    marketProvision: "Provision du market",
    provisionInviteNotice: "Le lien d'invitation est affiché uniquement après une réponse de provision réussie. L'envoi e-mail est marqué manuellement.",
    createMarketAndInvitation: "Créer le market et l'invitation",
    plan: "Plan d'abonnement",
    currentPlan: "Plan actuel",
    changePlan: "Changer de plan",
    savePlan: "Enregistrer le plan",
    planChanged: "Plan mis à jour.",
    monthlyCampaignQuota: "Quota de campagnes mensuelles",
    monthlyExportQuota: "Quota d'exports mensuels",
    privateProductLimit: "Limite de produits privés",
    privateTemplateLimit: "Limite de modèles privés",
    brandingAssetLimit: "Limite d'éléments de marque",
    unlimited: "Illimité",
    provisionPlan: "Plan de départ",
  },
  de: {
    platformAdmin: "Plattform-Admin",
    platformAdminDescription: "Sicherer Admin-Zugang für das LeafletPilot-Kontrollpanel.",
    email: "E-Mail",
    password: "Passwort",
    signIn: "Anmelden",
    signingIn: "Anmeldung läuft...",
    showPassword: "Passwort anzeigen",
    hidePassword: "Passwort ausblenden",
    invalidCredentials: "Ungültige E-Mail oder ungültiges Passwort",
    operationFailed: "Der Vorgang konnte nicht abgeschlossen werden.",
    overview: "Übersicht",
    logout: "Abmelden",
    pendingSignup: "Ausstehende Anmeldung",
    awaitingOwner: "Owner ausstehend",
    onboarding: "Einrichtung",
    readyMarket: "Market bereit",
    suspended: "Gesperrt",
    lifecycle: "Lebenszyklus",
    readiness: "Bereitschaft",
    ownerInvitation: "Owner-Einladung",
    activeUser: "Aktiver Benutzer",
    requiredSetup: "Erforderliche Einrichtung",
    lastActivity: "Letzte Aktivität",
    profile: "Profil",
    legalName: "Rechtlicher Name",
    location: "Standort",
    languageCurrency: "Sprache / Währung",
    timeZone: "Zeitzone",
    contact: "Kontakt",
    productCampaign: "Produkt / Kampagne",
    product: "Produkt",
    campaign: "Kampagne",
    member: "Mitglied",
    members: "Mitglieder",
    operation: "Betrieb",
    trialEnds: "Testphase endet",
    status: "Status",
    allStatuses: "Alle Status",
    market: "Market",
    markets: "Markets",
    marketList: "Market-Liste",
    open: "Öffnen",
    signupRequests: "Anfragen",
    signupRequestList: "Anfrageliste",
    signupOverview: "Kostenlose Testanfragen prüfen und passende Anfragen in Markets umwandeln.",
    marketOverview: "Lebenszyklus, Testphase und Einrichtung verfolgen.",
    search: "Suchen",
    contactPerson: "Kontakt",
    date: "Datum",
    phone: "Telefon",
    language: "Sprache",
    monthlyCampaigns: "Monatliche Kampagnen",
    notes: "Notizen",
    review: "In Prüfung setzen",
    reject: "Ablehnen",
    rejectionReason: "Ablehnungsgrund",
    activate: "Aktivieren",
    suspend: "Sperren",
    archive: "Archivieren",
    loading: "Wird geladen...",
    yes: "Ja",
    no: "Nein",
    complete: "Vollständig",
    missing: "Fehlt",
    unknownStatus: "Unbekannter Status",
    readinessReady: "Der Market ist bereit zur Veröffentlichung.",
    platformControl: "Plattformsteuerung",
    platformControlOverview: "Pilotkunden-Onboarding und Market-Bereitschaft verfolgen.",
    marketOperationsOverview: "Lebenszyklus, Bereitschaft, Owner-Einladung und Einrichtung verfolgen.",
    signupOperationsOverview: "Anfragen prüfen, genehmigen und in Pilot-Markets umwandeln.",
    allLifecycle: "Alle Lebenszyklen",
    allReadiness: "Alle Bereitschaftsstatus",
    noMarketsForFilters: "Keine Markets für diese Filter.",
    noSignupRequestsForFilters: "Keine Anfragen für diese Filter.",
    created: "Erstellt",
    updated: "Aktualisiert",
    lastUpdated: "Letzte Aktualisierung",
    lastReason: "Letzter Grund",
    reasonPrompt: "Grund eingeben",
    confirmArchiveMarket: "Diesen Market archivieren?",
    confirmStatusChangeSuffix: "anwenden?",
    confirmRevokeOwnerInvitation: "Ungenutzte Owner-Einladung widerrufen?",
    confirmRejectSignup: "Diese Anfrage ablehnen?",
    confirmProvisionSignup: "Market und Owner-Einladung für diese Anfrage erstellen?",
    expires: "Ablauf",
    accepted: "Angenommen",
    ownerEmail: "Owner-E-Mail",
    createInvitation: "Einladung erstellen",
    rotateInvitation: "Einladung erneuern",
    revokeInvitation: "Einladung widerrufen",
    oneTimeInviteLink: "Einmaliger Einladungslink",
    lastSent: "Zuletzt gesendet",
    sendCount: "Sendeanzahl",
    sendError: "Sendefehler",
    invitationEmailNotice: "Der Einladungslink wird per E-Mail gesendet; rohe Tokens werden nicht angezeigt.",
    copy: "Kopieren",
    copyInvitationLink: "Einladungslink kopieren",
    manualLinkCopied: "Einladungslink kopiert.",
    createInvitationLink: "Einladungslink erstellen",
    manualDeliveryNotice: "Der E-Mail-Versand ist deaktiviert. Erstellen Sie den sicheren Einladungslink und teilen Sie ihn manuell mit dem Owner.",
    manualLinkNotFound: "Keine nutzbare Einladung gefunden. Aktualisieren oder erneuern Sie die Einladung und versuchen Sie es erneut.",
    manualLinkConflict: "Eine manuelle Zustellung ist nicht mehr erforderlich. Aktualisieren Sie den Einladungsstatus.",
    manualLinkFailed: "Der sichere Einladungslink konnte nicht erstellt werden. Bitte versuchen Sie es erneut.",
    none: "Keine",
    activateOrContinue: "Aktivieren / Fortsetzen",
    step: "Schritt",
    platformActivity: "Plattformaktivität",
    recentPlatformActivity: "Letzte Plattformaktivität",
    action: "Aktion",
    target: "Ziel",
    noActivity: "Keine Aktivität.",
    noPlatformActivity: "Noch keine Plattformaktivität.",
    reviewNotes: "Prüfnotizen",
    approve: "Genehmigen",
    marketProvision: "Market-Provisionierung",
    provisionInviteNotice: "Der Einladungslink wird nur nach einer erfolgreichen Provisionierungsantwort angezeigt. E-Mail-Versand wird manuell markiert.",
    createMarketAndInvitation: "Market und Einladung erstellen",
    plan: "Abonnementplan",
    currentPlan: "Aktueller Plan",
    changePlan: "Plan ändern",
    savePlan: "Plan speichern",
    planChanged: "Plan aktualisiert.",
    monthlyCampaignQuota: "Monatliches Kampagnenkontingent",
    monthlyExportQuota: "Monatliches Exportkontingent",
    privateProductLimit: "Limit für private Produkte",
    privateTemplateLimit: "Limit für private Vorlagen",
    brandingAssetLimit: "Limit für Markenelemente",
    unlimited: "Unbegrenzt",
    provisionPlan: "Startplan",
  },
};

export const statusLabels = {
  tr: {
    pending: "Bekliyor",
    reviewing: "İnceleniyor",
    under_review: "İnceleniyor",
    approved: "Onaylandı",
    rejected: "Reddedildi",
    provisioned: "Kuruldu",
    trial: "Deneme",
    active: "Aktif",
    suspended: "Askıya alındı",
    archived: "Arşivlendi",
    awaiting_owner: "Owner bekleniyor",
    onboarding: "Kurulumda",
    not_started: "Başlamadı",
    in_progress: "Devam ediyor",
    ready: "Hazır",
    blocked: "Engelli",
    completed: "Tamamlandı",
    revoked: "İptal edildi",
    expired: "Süresi doldu",
    manual: "Manuel",
    manual_delivery_required: "Manuel teslim gerekli",
    sent: "Gönderildi",
    failed: "Başarısız",
    accepted: "Kabul edildi",
  },
  en: {
    pending: "Pending",
    reviewing: "Reviewing",
    under_review: "Reviewing",
    approved: "Approved",
    rejected: "Rejected",
    provisioned: "Provisioned",
    trial: "Trial",
    active: "Active",
    suspended: "Suspended",
    archived: "Archived",
    awaiting_owner: "Awaiting owner",
    onboarding: "Onboarding",
    not_started: "Not started",
    in_progress: "In progress",
    ready: "Ready",
    blocked: "Blocked",
    completed: "Completed",
    revoked: "Revoked",
    expired: "Expired",
    manual: "Manual",
    manual_delivery_required: "Manual delivery required",
    sent: "Sent",
    failed: "Failed",
    accepted: "Accepted",
  },
  fr: {
    pending: "En attente",
    reviewing: "En revue",
    under_review: "En revue",
    approved: "Approuvé",
    rejected: "Rejeté",
    provisioned: "Provisionné",
    trial: "Essai",
    active: "Actif",
    suspended: "Suspendu",
    archived: "Archivé",
    awaiting_owner: "Owner attendu",
    onboarding: "Configuration",
    not_started: "Non commencé",
    in_progress: "En cours",
    ready: "Prêt",
    blocked: "Bloqué",
    completed: "Terminé",
    revoked: "Révoqué",
    expired: "Expiré",
    manual: "Manuel",
    manual_delivery_required: "Livraison manuelle requise",
    sent: "Envoyé",
    failed: "Échec",
    accepted: "Accepté",
  },
  de: {
    pending: "Ausstehend",
    reviewing: "In Prüfung",
    under_review: "In Prüfung",
    approved: "Genehmigt",
    rejected: "Abgelehnt",
    provisioned: "Eingerichtet",
    trial: "Testphase",
    active: "Aktiv",
    suspended: "Gesperrt",
    archived: "Archiviert",
    awaiting_owner: "Owner ausstehend",
    onboarding: "Einrichtung",
    not_started: "Nicht begonnen",
    in_progress: "In Bearbeitung",
    ready: "Bereit",
    blocked: "Blockiert",
    completed: "Abgeschlossen",
    revoked: "Widerrufen",
    expired: "Abgelaufen",
    manual: "Manuell",
    manual_delivery_required: "Manuelle Zustellung erforderlich",
    sent: "Gesendet",
    failed: "Fehlgeschlagen",
    accepted: "Angenommen",
  },
};

export const blockerLabels = {
  tr: {
    active_user_missing: "Aktif kullanıcı bulunmuyor",
    owner_invitation_pending: "Owner daveti henüz kabul edilmedi",
    onboarding_incomplete: "Onboarding tamamlanmadı",
    product_required: "En az bir ürün eklenmeli",
    campaign_required: "En az bir kampanya oluşturulmalı",
    profile_incomplete: "Market profili eksik",
    market_suspended: "Market askıya alınmış",
    required_setup_incomplete: "Gerekli market kurulumu tamamlanmadı",
  },
  en: {
    active_user_missing: "No active user found",
    owner_invitation_pending: "Owner invitation has not been accepted yet",
    onboarding_incomplete: "Onboarding is not complete",
    product_required: "At least one product must be added",
    campaign_required: "At least one campaign must be created",
    profile_incomplete: "Market profile is incomplete",
    market_suspended: "Market is suspended",
    required_setup_incomplete: "Required market setup is not complete",
  },
  fr: {
    active_user_missing: "Aucun utilisateur actif trouvé",
    owner_invitation_pending: "L'invitation owner n'a pas encore été acceptée",
    onboarding_incomplete: "La configuration n'est pas terminée",
    product_required: "Au moins un produit doit être ajouté",
    campaign_required: "Au moins une campagne doit être créée",
    profile_incomplete: "Le profil du market est incomplet",
    market_suspended: "Le market est suspendu",
    required_setup_incomplete: "La configuration requise du market n'est pas terminée",
  },
  de: {
    active_user_missing: "Kein aktiver Benutzer vorhanden",
    owner_invitation_pending: "Die Owner-Einladung wurde noch nicht angenommen",
    onboarding_incomplete: "Die Einrichtung ist nicht abgeschlossen",
    product_required: "Mindestens ein Produkt muss hinzugefügt werden",
    campaign_required: "Mindestens eine Kampagne muss erstellt werden",
    profile_incomplete: "Das Market-Profil ist unvollständig",
    market_suspended: "Der Market ist gesperrt",
    required_setup_incomplete: "Die erforderliche Market-Einrichtung ist nicht abgeschlossen",
  },
};

const englishBlockerAliases = {
  "Required market setup is not complete.": "required_setup_incomplete",
  "No active owner user or effective owner invitation.": "owner_invitation_pending",
  "Market lifecycle is suspended.": "market_suspended",
  "Market lifecycle is archived.": "market_suspended",
};

function normalizeLocale(locale) {
  return platformLocales.includes(locale) ? locale : "tr";
}

export function t(key, locale = "tr") {
  const normalized = normalizeLocale(locale);
  return platformLabels[normalized][key] || platformLabels.tr[key] || key;
}

export function statusLabel(value, locale = "tr") {
  const normalized = normalizeLocale(locale);
  if (typeof value !== "string" || !value) return t("unknownStatus", normalized);
  return statusLabels[normalized][value] || `${t("unknownStatus", normalized)}: ${value}`;
}

export function blockerCode(blocker) {
  if (typeof blocker === "string") return englishBlockerAliases[blocker] || blocker;
  if (blocker && typeof blocker === "object" && typeof blocker.code === "string") return blocker.code;
  return "unknown";
}

export function blockerLabel(blocker, locale = "tr") {
  const normalized = normalizeLocale(locale);
  const code = blockerCode(blocker);
  return blockerLabels[normalized][code] || `${t("unknownStatus", normalized)}: ${code}`;
}

export function deriveReadiness(market) {
  const blockers = [];
  const productCount = Number(market?.product_count || 0);
  const campaignCount = Number(market?.campaign_count || 0);
  const memberCount = Number(market?.member_count || 0);

  if (market?.readiness) {
    for (const blocker of market.readiness.blockers || []) {
      blockers.push({ code: blockerCode(blocker) });
    }
    if (!market.readiness.has_active_market_user && !market?.owner_invitation?.is_effective) {
      blockers.push({ code: "owner_invitation_pending" });
    }
    if (market.onboarding_status && market.onboarding_status !== "completed") {
      blockers.push({ code: "onboarding_incomplete" });
    }
    if (!market.readiness.required_setup_complete && productCount < 1) {
      blockers.push({ code: "product_required" });
    }
    if (!market?.legal_name || !market?.country_code || !market?.language || !market?.currency || !market?.timezone) {
      blockers.push({ code: "profile_incomplete" });
    }
    return { status: market.readiness.state || "blocked", blockers: uniqueBlockers(blockers) };
  }

  if (market?.lifecycle_status === "suspended" || market?.lifecycle_status === "archived") {
    blockers.push({ code: "market_suspended" });
  }
  if (memberCount < 1) blockers.push({ code: "active_user_missing" });
  if (market?.onboarding_status && market.onboarding_status !== "completed") {
    blockers.push({ code: "onboarding_incomplete" });
  }
  if (productCount < 1) blockers.push({ code: "product_required" });
  if (campaignCount < 1) blockers.push({ code: "campaign_required" });
  if (!market?.legal_name || !market?.country_code || !market?.language || !market?.currency || !market?.timezone) {
    blockers.push({ code: "profile_incomplete" });
  }

  const status = blockers.length === 0 ? "ready" : market?.onboarding_status === "completed" ? "blocked" : "onboarding";
  return { status, blockers: uniqueBlockers(blockers) };
}

function uniqueBlockers(blockers) {
  const seen = new Set();
  return blockers.filter((blocker) => {
    const code = blockerCode(blocker);
    if (seen.has(code)) return false;
    seen.add(code);
    return true;
  });
}

export function countRows(market, locale = "tr") {
  return [
    { label: t("product", locale), value: Number(market?.product_count || 0) },
    { label: t("campaign", locale), value: Number(market?.campaign_count || 0) },
  ];
}
