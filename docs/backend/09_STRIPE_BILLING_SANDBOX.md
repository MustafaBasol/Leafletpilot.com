...
## Plan değişikliği sonrası UI senkronizasyonu (frontend, `src/pages/Billing.jsx`)

`POST /billing/change-plan`'ın `{"status": "applied"}` dönmesi, Stripe mutasyonunun kabul edildiği anlamına gelir — yerel `MarketSubscription` satırı **hâlâ yalnızca** ilgili webhook (`customer.subscription.updated`) işlendiğinde yazılır (bkz. dosyanın en üstündeki çekirdek kural). `{"status": "scheduled"}` (düşürme) durumunda satırı yazan webhook değil aynı istek işleyicisidir, ama mutasyon yanıtından hemen sonraki bir okuma yine de bu yazımın commit görünürlüğünü geçebilir. Bu yüzden `confirmPlanChange` hiçbir mutasyon sonrasında tek, kontrolsüz bir `load()`'a güvenmez:

- **`pending_payment`**: plan henüz değişmemiştir (ödeme onayı bekliyor) — modal hemen kapanır, ilgili bilgi mesajı gösterilir, tek bir `load()` yeterlidir (durum zaten değişmediği için doğrulanacak bir hedef state yok).
- **`scheduled` (düşürme)**: mevcut plan **değişmez** — sadece `pending_plan_code`/`pending_change_at` set edilir, düşürme hiçbir zaman anlık bir entitlement değişikliği olarak ele alınmaz. Modal yine de "senkronize ediliyor" durumuna geçer ve **sınırlı** bir yeniden-çekme döngüsü çalışır (yükseltmeyle aynı `PLAN_SYNC_MAX_ATTEMPTS`/`PLAN_SYNC_RETRY_DELAY_MS`), ancak başarı koşulu farklıdır: `subscription.plan_code === currentPlanCode` **ve** `subscription.pending_plan_code === targetPlanCode` **ve** `subscription.pending_change_at` dolu — asla `plan_code === targetPlanCode` değil (mevcut plan asla hedefe eşitlenmez).
...
- Sınırlı deneme tükenip hâlâ senkronize olmadıysa (yükseltme veya düşürme fark etmeksizin): işlenmekte olduğunu belirten güvenli bir mesaj ve yanında manuel bir "Yenile" aksiyonu (`handleManualSync`) gösterilir — kullanıcı tam sayfa yenilemeye asla ihtiyaç duymaz.
