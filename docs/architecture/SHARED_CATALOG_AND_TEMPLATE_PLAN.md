...
The deterministic Phase F seed and fixed-port Playwright harness are now present at `backend/scripts/seed_phase_f.py` and `scripts/phase_f_playwright.py`; the harness owns a disposable PostgreSQL 16 container, runs migrations, repeats the seed, performs authenticated API preflight, captures process/browser diagnostics, and targets `8200`/`4273`. The fixture root cause was immediate flushing of market-scoped templates with the model default `is_global=true`, followed by unstable password-hash lookup for admin/user reruns. The seed now uses explicit market scope and stable email lookup, emits a secret-free JSON self-check, and passes twice with 3 markets, 2 users, 6 templates, 1 product, 3 market products, 3 campaigns, 3 campaign items, and 2 export jobs. The campaign detail UI now exposes scoped product reorder controls and a finalize/freeze action backed by the existing immutable API contract. The passing smoke gate records 16 screenshots, three builder products, zero console errors, zero page errors, zero failed critical requests, and passing auth/builder-options/campaign preflight. The remaining limitation is that the browser script does not yet drive and assert every requested UI mutation (ordering persistence, slot validation, freeze stability after source mutation, export downloads/idempotency, entitlement limit, historical rendering, preview/export parity, and market isolation); Phase F remains incomplete and Phase G is not safe. The exact pushed HEAD `e637e73` has backend CI failure because the seed-only regression test was not fixture-aware; frontend and Docker passed. PR #25 remains draft; review submissions and inline review threads are currently zero.
